...
%Here, we present the lifespans of Groups and discuss Walls/Albums in \autoref{sec:walls-albums}.
...
\item $\vard{\eHasModerator}_{\mathsf{g}} \in \interval{ \varc{\eHasModerator}_{\mathsf{g}} + \Delta}{\min (\vard{\eForum}_{\mathsf{g}}, \vard{\ePerson})}$ %,\xNC
\end{itemize}
...
\begin{itemize}
    \item $\varc{\eHasMember}_\textsf{a} = \max ( \varc{\eForum}_\textsf{a}, \varc{\eKnows}_{i,j} ) + \Delta $ %, where $\varc{\eKnows}_{i,j}$ is the creation date of the \tKnows edge between $\ePerson_i$ and $\ePerson_j$.
    \item $\vard{\eHasMember}_{\mathsf{w}} = \min ( \vard{\eForum}_\textsf{a}, \vard{\eKnows}_{i,j} ) $ %, where $\vard{\eKnows}_{i,j}$ is the deletion date of the \tKnows edge between $\ePerson_i$ and $\ePerson_j$.
\end{itemize}
...
%is the creation date of the \tHasMember edge between
%$\varc{\eHasMember}$ 
...
These events span a $2\phi$-hour time window centered around a specific event time, %\tFlashmobEvent
...
% \paragraph{Photo}
% \begin{itemize}
%     \item $\varc{\ePost} \in \interval{\varc{\eHasMember} + \Delta}{\min (\vard{\eHasMember}, \xSE) }$
%     % where $\varc{\eHasMember}$ is the creation date of the \tHasMember edge between \tPerson $\ePerson$ and \tForum $\eForum$.
%     \item $\vard{\ePost} \in \interval{\varc{\ePost} + \Delta}{\min (\vard{\eHasMember}, \xNC)}$
% \end{itemize}
...
%SNB treats some \tPost nodes as \emph{Photos}. This is a non-trivial -- here is some background information from our documents:

% \begin{itemize}
%     \item The SNB specification states that ``\textsf{Photos} contain either \texttt{content} or \texttt{imageFile}, always one of them but never both.''
%     \item The graph schema does not distinguish \textsf{Photos} from \textsf{Posts}.
%     \item The current Datagen implementation treats \textsf{Photos} as a subclass of \tMessage (\texttt{Photo extends Message}) for simplicity.
%     \item Based on the code, Photos do not have any \textsf{Comments}.
% \end{itemize}

\subsubsection{Comment}

A \tComment $\eComment$ is created by \tPerson $\ePerson$ as a reply to \tMessage $\eMessage$. \tComments are only made in Walls and Groups. \tComment always occur within %1
$\gamma$~days of their parent message. % following a power-law distribution with mean 6.85 hours.
