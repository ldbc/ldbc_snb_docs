\begin{table}[htb]
    ...
    \begin{tabularx}{\linewidth}{|>{\sffamily}c|>{\tt}l|>{\tt}X|}
        ...
        \hline
    \end{tabularx}
    \caption{Files output by the \texttt{csv-composite-merged-fk} serializer (18 in total). The first part of the table contains the static entities, the second part contains the dynamic ones.
        Note that the \textsf{ParentPostId} and the \textsf{ParentCommentId} columns together correspond to the \textsf{replyOf} edge in the schema.
