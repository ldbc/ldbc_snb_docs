...
PostgreSQL defines the \lstinline[language=sql]{OVER} keyword to use aggregation functions as window functions.\footnote{\url{https://www.postgresql.org/docs/9.1/static/tutorial-window.html}}
...
Numerous use cases require \emph{composition} of queries, including the reuse of query results (\eg nodes, edges) and using scalar subqueries.
...
Handling dates and times is a fundamental requirement for database systems.
...
\paragraph{Note on terminology.} The \emph{Glossary of graph theory terms} page of Wikipedia\footnote{\url{https://en.wikipedia.org/wiki/Glossary_of_graph_theory_terms}} defines \emph{paths} as follows: ``A path may either be a walk (a sequence of vertices and edges, with both endpoints of an edge appearing adjacent to it in the sequence) or a simple path (a walk with no repetitions of vertices or edges), depending on the source.''
In this work, we use the first definition, which is more common in modern graph database systems and is also followed in a recent survey on graph query languages~\cite{Angles:2017:FMQ:3145473.3104031}. In other words, we use the expressions ``path'' and ``walk'' interchangeably.

Handling paths is one of the key distinguishing features of graph database systems.

%RPQ (regular path queries)

\paragraph{Node and edge uniqueness.} Some use cases specify that paths must not have repeated edges (in graph theory, these walks are called ``trails''). More formally, paper~\cite{Angles:2017:FMQ:3145473.3104031} defines \emph{homomorphism-based semantics} (no constraints on repetitions) and
multiple flavours of \emph{isomorphism-based semantics}:
\begin{itemize}
\item \emph{no-repeated-node semantics} (also called \emph{fully isomorphic matching}),
\item \emph{no-repeated-edge semantics} (also called \emph{edge-isomorphic matching}),
\item \emph{no-repeated-anything semantics} (not used in the context of this work).
\end{itemize}
...
RPQs are also proposed in the openCypher language as \emph{path patterns}.\footnote{\url{https://github.com/thobe/openCypher/blob/b95eec108ce4ec07eedfe13b3e5fff0e94f789a4/cip/1.accepted/CIP2017-02-06-Path-Patterns.adoc}}

\item[G-CORE.] The \mbox{G-CORE} language~\cite{GCORE_SIGMOD} treats paths as \emph{first-order citizens}: its \emph{path property graph data model} can store paths in the graph model with labels and properties. However, it only supports shortest path semantics (for tractability reasons) and does not allow enumeration of all paths. \mbox{G-CORE} uses \emph{homomorphism-based matching semantics}.
