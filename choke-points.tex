...
\cpSection{1.3}{QOPT}{Top-k push down}
...
In this work, we use the first definition, which is more common in modern graph database systems and is also followed in a recent survey on graph query languages~\cite{Angles:2017:FMQ:3145473.3104031}.

Handling paths as first-class citizens is one of the key distinguishing features of graph database 
systems~\cite{GCORE_SIGMOD}. Hence, additionally to
...
of the path.
Also, some use cases specify uniqueness constraints on paths, \eg that a certain path must
not have repeated nodes (referred to as ``walks'' in graph theory) or 
not have repeated edges (``trails'' in graph theory). More precisely, 
paper~\cite{Angles:2017:FMQ:3145473.3104031} defines \emph{homomorphism-based semantics} 
(no constraints on repetitions) and multiple flavours of \emph{isomorphism-based semantics}:
\begin{itemize}
\item \emph{no-repeated-node semantics} (also known as \emph{fully isomorphic matching}),
\item \emph{no-repeated-edge semantics} (also known as \emph{edge-isomorphic matching}),
