...
In general, online transaction processing (OLTP) benchmarks allow implementation via stored procedures, effectively amounting to explicit query plans.\footnote{Consider for example \mbox{TPC-C}, where implementations are known to specify join orders and indices.}
...
Additionally, if a benchmark is analytical and a query is stated using a declarative language, explicit control of join order, join type, \etc should be prohibited.
...
Therefore, the LDBC situation regarding declarativity is not as simple as that of for example the \mbox{TPC-H} where queries should be specified in SQL with the additional constraint of omitting any hints for OLAP workloads.

Individual benchmarks are expected to specify their policy of either using a domain-specific query language or implementing the queries in a general-purpose programming language.
...
The \emph{validation run} shall be performed on the SF10 dataset (see \autoref{sec:validation-data-set}). Note that the auditor may perform additional validation runs of the benchmark implementation using smaller datasets (\eg SF1).
...
LDBC provides a test driver, data generator, and summary reporting scripts. Benchmark implementations shall use a stable version (\eg 0.3.4) of the test driver.
...
Using stored procedures is not allowed except for the ``standard libraries'' available for the SUT. (These libraries often include features such as weighted shortest path algorithms.)

Checking whether an implementation is only using a domain-specific language and its standard stored procedure libraries is the responsibility of the author.

\subsubsection{Query variants}

Several queries (\eg \queryRefCard{bi-read-14}{BI}{14}) use multiple variants with different sets of input parameters.
These should be run in a mix and the SUT should not be informed on which variant it is currently evaluating (\eg 14(A) or 14(B)).
