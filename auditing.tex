...
In each batch, each query variant (Q1, Q2a, Q2b, Q3, \ldots, Q20a, Q20b) is executed using 30~different substitution parameters.
The benchmark includes two tests which use different workflows in their batches.

\begin{itemize}
    \item The \emph{power test} first runs the write operations, followed by sequential execution of individual read query variants.
    \item The \emph{throughput test} performs the same operations, while allowing concurrent execution of the write operations and read queries required in a given batch.
...
We define two execution modes, \emph{disjoint RW mode} and \emph{concurrent RW mode} for the workload.
The execution of the \emph{power batch} is identical for both modes.
During the excution of the \emph{throughput batch}:

\begin{itemize}
    \item In \textbf{disjoint RW mode} the system performs the reads and writes separately: first the writes followed by the reads. Concurrency between the read operations is possible. This mode is aimed at \emph{read-optimized data analytical systems} do not support concurrent reads and writes. Implementations may also opt to use this mode for simplicity. For these implementations, passing the \emph{LDBC ACID compliance test} (\autoref{sec:acid-test-suite}) is not required.
    \item In \textbf{concurrent RW mode} the system is allowed to run reads and writes concurrently. This requires the systems to be capable of handling \emph{transactions}. Implementations using this mode are required to pass the \emph{LDBC ACID compliance test} (\autoref{sec:acid-test-suite}).
\end{itemize}

Results with these systems are reported in the same leaderboard on the LDBC website.
...
The main part of the BI workflow (\autoref{fig:bi-auditing-workflow}) consists of two parts.

\begin{itemize}
    \item Running the \textbf{power test}, \ie apply one day's write batch, then run the reads sequentially.
...
During the \emph{throughput test}, systems capable of transactional features (\autoref{sec:bi-target-systems}) can opt to run read and write operations concurrently (\emph{concurrent RW mode}).
...
In cases when the exact approach used by the SUT's query implementations is uncertain, it is the auditor's responsibility to decide whether the SUT complies with this rule.
...
The rationale behind this decision is to ensure that there should be a sufficient number of write operations available to guarantee the execution during the duration of the measurement window (see \autoref{sec:bi-benchmark-workflow}).
