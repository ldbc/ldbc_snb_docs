...
These systems should guarantee at least \texttt{TODO} isolation level and guarantee \emph{ACID compliance} (\autoref{sec:acid-compliance}).
...
\label{sec:bi-primary-metric}

The primary metric of the BI worklaod is the \emph{number of batches executed per day} ($\mathit{bpd}$).
...
For example, if the SUT took $3.5\text{ hours}$ to load, executed the fully completed throughput batches for $1.75$ hours and completed $7$ batches:
$$
