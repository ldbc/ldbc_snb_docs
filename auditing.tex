...
\section{Business Intelligence workload}
...
The rationale behind this decision is to ensure that there is a sufficient number of update operations available to guarantee XX~hours of continuous execution (see \autoref{sec:bi-benchmark-workflow}).
...
Moreover, it is not allowed for the query implementations to contain code that aims to detects the query variant used.

\subsection{Benchmark Workflow}
\label{sec:bi-benchmark-workflow}

The \emph{load time} ($t_\textit{load}$) shall include the time to load the data into the SUT and initialize auxiliary data structures (if applicable).

The \emph{measurement window} includes running the W/R batches.
...
During the measurement window, the benchmark should run $n_\textit{batches}$ complete W+R batches for a total of $t$ time ($n_\textit{batches} \geq 2$ and $t_\textit{batches} \geq 2\text{ hours}$).
Then, the metric is determined as:
$$
\mathit{bpd} = (24 - t_\textit{load}) \times \frac{n_\textit{batches}}{t_\textit{batches}}
$$
...
\label{sec:bi-execution-rules}

\begin{itemize}
    \item The benchmark shall implement all operators correctly.
    \item The benchmark shall run for at least 1 hour and include at least two full batches (writes and reads).
    \item No ``warmup'' period is allowed.
\end{itemize}
...
\begin{itemize}
    \item Using auxiliary data structures (\eg indexes, materialized views) is allowed if they are kept in an up-to-date state after each write batch.
    \item The SUT shall pass the ACID test suite (\autoref{sec:acid-test-suite}). TODO: this needs more rationale given that the BI workload can be implemented on Spark-like systems
\end{itemize}
...
including the ACID compliance report (\autoref{sec:acid-compliance}).
