...
The scale factor of a SNB dataset is the size of the dataset in GiB of CSV (comma-separated values) files. All dataset scales contain data for three years of social network activity. The scaling is done in buckets. The size of the dataset is either 10~GiB or 30~GiB or a power of ten multiple of either. Thus scales are 10, 30 100, 300, 1000~GiB and so forth. This scaling is identical to the scaling of TPC-H and TPC-DS. 
...
\begin{itemize}
    ...
    \item SUT-specific LDBC Java driver implementation (similarly to the projects in \url{https://github.com/ldbc/ldbc_snb_implementations})
    ...
    \item The implementations of the queries (IC/IS) and the update operations, including the query specifications (\eg declarative queries, stored procedures, \etc)
