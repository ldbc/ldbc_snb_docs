...
% \item Consistency: If a database object, \eg table, has auxiliary data structures, \eg indices, the content of these
% must be consistent after the commit or rollback of a transaction. If multiple client application threads share
% one transaction context, these may transiently see inconsistent states, \eg there may be a time when an
...
\item[Serialisable] The transactions see values that correspond to a fully serial execution of
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.9\linewidth]{figures/auditing-workflow}    
    \caption{Auditing workflow.}
    \label{fig:audit-workflow}
\end{figure}

\emph{An additional resource about the LDBC SNB Benchmark Auditing Process is available in~\cite{auditing}.}
...
The update stream contains the latest 10\% of the events in the simulated social network. These events form a single serialisable sequence in time. Some events will depend on preceding events, for example a message must exist before a reply comment to the message is created. The data generator guarantees that these are separated by at least 10 seconds of simulation time.
...
Imperative implementations are expected to respect the rules described in \autoref{sec:query-declarativity}. For these implementations, the rules in \autoref{sec:snb-declarative-implementations} do not apply.
...
% TODO what does it mean? What consistency level is expected? Is it going to be tested at all?
...
% TODO this was here in the original list for isolation
\paragraph{Expected level of isolation}
If a transaction reads the database with intent to update, the DBMS must guarantee that repeating the same read within the same transaction will return the same data. This also means that no more and no less data rows must be returned. In other words, this corresponds to snapshot or to serialisable isolation. If the database is accessed without transaction context or without intent to update, then the DBMS should provide read committed semantics, \eg repeating the same read may produce different results but these results may never include effects of pending uncommitted transactions.


\paragraph{Durability and Checkpoints}
% TODO initially this was way below this point; it was the last part of the benchmark workflow, so it uses definitions from the upcoming parts
...
The SNB \datagen produces 3~years worth data of which 10\% is used for updates (\autoref{sec:sn-datasets}).
...
% TODO add precisely the outline of FDR and a list of supplementary material?
