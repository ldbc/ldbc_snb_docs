...
\subsection{Chellenge Procedure}
...
TODO: it is allowed to implement queries as a sequence of multiple queries (e.g. create projected graph, run query, drop projected graph).
...
TODO: load time: load the data set, initialize auxiliary data structures.

TODO: measurement window at least 1 hour of R/W batches.
...
During the benchmark's execution, the benchmark should run $b$ batches for a total of $t$ time ($b \geq 2$ and $t > 1\text{ hour}$).
