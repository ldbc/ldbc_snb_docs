...
\begin{itemize}
    ...
    \item Allowing concurrent reads and writes (\autoref{sec:execution-modes}) (optional, only alowed if ACID compliance is guaranteed)
\end{itemize}
...
The \emph{throughput test} consists of multiple \emph{throughput batches}. These perform the same operations as the power batch but (optionally) allow concurrent execution of the write operations and read queries in a given batch.
Each throughput batch runs the same amount of operations as the power batch.
...
\paragraph{Disjoint RW mode}
In \emph{disjoint RW mode} the system performs the reads and writes separately. It first executes the writes, then evaluates the read queries. Concurrency between the read operations is allowed.\\
...
\paragraph{Concurrent RW mode}
In \emph{concurrent RW mode} the system is allowed to run reads and writes concurrently. This requires the systems to be capable of handling \emph{transactions}. Implementations using this mode are required to pass the \emph{LDBC ACID compliance test} (\autoref{sec:acid-test-suite}).
