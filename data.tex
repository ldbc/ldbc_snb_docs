...
For both workloads, we use the default settings for the splitting the data into an initial (bulk-loaded) data set and the later update operations (``streams'').
...
\begin{description}
    ...
    \emph{Map hierarchy to single table} is used, \ie all Organisation nodes are serialized in a single file. A discriminator attribute ``type'' is used with the value denoting the concrete class, \eg ``Company''.
\end{description}
...
As illustrated later, this interval is often used to determine the intervals where the creation and deletion dates of dependent entities are selected.
