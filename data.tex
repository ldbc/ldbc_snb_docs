...
Alternatively, at any time up until this event, the friendship can be explicitly deleted, \ie two people have a disagreement and “unfriend” each other, but both continue using the social network.
...
The probability distributions used to determine if a dynamic entities is explicitly deleted and then when that event occurs is discussed in~\autoref{sec:ensuring-realism}.
...
Paper \cite{DBLP:conf/cscw/AlmuhimediWLSA13} tracked 292K Twitter users for 1 week finding 2.4\% of 67.2M tweets were deleted across 4 categories: status posts, retweets, replies, and mentions of other users that were not replies.
...
Additionally, paper~\cite{DBLP:conf/cscw/AlmuhimediWLSA13} identified not all users delete messages, with around 50\% of users doing so.
...
Almuhimedi et al. (2013) observe a temporal relationship for when a tweet is deleted.
