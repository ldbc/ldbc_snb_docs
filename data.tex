...
This is then used to determine the probability a person is deleted, using the distribution given below.
A deletion date is then selected uniformly from the person's lifespan.
...
As Datagen models a generic social media platform we have chosen a different ratio of 20:1, rather than overcapture behavior that may be unique to a single site.
They also find a constant background flux of follows and unfollows interleaved with bursts in such activity.
...
\todo{add ref}
Almuhimedi et al. (2013) tracked 292K Twitter users for 1 week finding 2.4\% of 67.2M tweets were deleted across 4 categories: status posts, retweets, replies, and mentions of other users that were not replies.
In order to apply these findings to Datagen, tweet categories were mapped to Datagen message types.
The table below shows the mapping and the percentage deleted within each category.
\todo{add table}
Additionally, Almuhimedi et al. (2013) identified not all users delete messages, with around 50\% of users doing so.
Thus, each person in the network has a 50\% chance of being marked a “deleter”, who subsequently, may or may not, delete messages or likes.

Almuhimedi et al. (2013) also identify a relationship between the depth of comments on a post and the chance the post is removed, less comments, more likely a post is deleted.
We model this relationship using the distribution above.
\todo{add plot}
Posts in groups and walls are produced via a uniform generator and a flash mob generator, capturing background events and bursts in events respectively. A comment generator is then used to produce a tree of comments on each post.
Posts in albums are referred to as photos. They are produced by a different generator and do not have flash mob events nor do they have comment trees.
...
We have recreated this distribution and use it to generate deletion dates for posts, comments, and likes.
\todo{add plot}
