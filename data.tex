...
\item Group membership is composed of 30\% from the moderator's friends and the remainder is chosen randomly (from the block the person is in).\todo{what is a block? explain}
...
\input{tables/table-csv-basic}

\input{tables/table-csv-merge-foreign}

\input{tables/table-csv-composite}

\input{tables/table-csv-composite-merge-foreign}

\input{tables/table-csv-basic-rawdata}
...
The filenames (without the extension) end in \texttt{\_i\_j} where \texttt{i} is the reducer id and \texttt{j} is the partitioner id.
...
The following CSV variants are supported:
    \begin{itemize}
      ...
      (1)~additional attributes (\eg \texttt{deletionDate}, \texttt{isExplicitlyDeleted}, \texttt{weight}) are included mainly for debugging,
...
\input{tables/table-update-streams}
...
To understand common behaviour of deletes we informally surveyed several social networks, the findings of which motivated the design of 8 delete operations described in~\autoref{sec:bi-refreshes}.

The next step was to generate \emph{delete events} within LDBC's synthetic data generator and ensure that they follow a logic order in the social network.
For example, a delete \tKnows edge event can only occur after both \tPersons join the network and become friends, but before either \tPerson leaves the network.
...
Deriving valid lifespans for dynamic entities was the subject of a short paper published at the GRADES-NDA~2020 workshop~\cite{DBLP:conf/sigmod/WaudbySPS20} and is presented in~\autoref{sec:lifespan-management}.

Next it was important to distinguish between \emph{implicit} and \emph{explicit} delete events.
Continuing with the \tKnows edge example, once created the connection exists until either \tPerson leaves the network, at which point the connection is implicitly deleted, as per the semantics of delete \tPerson~(\autoref{sec:bi-delete-01}).
Alternatively, at any time up until this event, the friendship can be explicitly deleted, i.e., two people have a disagreement and “unfriend” each other, but both continue using the social network.
Distinguishing between these types of events is important as only explicit delete events should become delete operations in the workload.

To achieve this each dynamic entity is assigned a probability of being explicitly deleted, if selected the entity is marked as such; this is used to ensure the correct serialization of delete events into delete operations.
...
To achieve this each dynamic entity is assigned a realistic distribution to select delete event timestamps from, which respects the bounds imposed by the valid lifespans.
The probability distributions used determine if a dynamic entities is explicitly deleted and then when that event occurs is discussed in~\autoref{sec:ensuring-realism}.
...
Firstly, each dynamic is assigned a probability of being explicitly deleted.
Second, if selected for explicit deletion, a deletion date is selected using a distribution bound by the valid lifespan of that entity.
To make informed choices of deletion probabilities and deletion date distribution where possible real-world data was used.

\paragraph{Delete Person}

\cite{Lorincz2019} identifies people with more connections are less likely to leave a social network.
When a \tPerson is generated they are assigned a \emph{maxKnows} value which indicates the amount of \tKnows connections they will make across the lifetime of the network.
This is then used to determine the probability a person is explicitly deleted, using the distribution given below.
A deletion date is then selected uniformly from the person's valid lifespan.
On average 3.5\% of \tPersons are deleted across the simulation period.
\todo{add plot}

\paragraph{Delete Knows}

\cite{DBLP:conf/www/MyersL14} analysed 1.2 billion tweets from 13.1 million Twitter users.
These users made 112.3 million new connections, and deleted 39.2 million connections; a 3:1 follow:unfollow ratio.
As Datagen models a generic social media platform we have chosen a different ratio of 20:1 (on average 5\% of \tKnows edges are deleted), rather than overcapture behavior that may be unique to a single site.
...
Currently, Datagen has no follow bursts, thus, we have decided not to incorporate follow bursts.
They also find less similar friends have a high probability of being unfollowed.
\todo{modelling this is work in progress}
If a \tKnows edge is selected for explicit deletion then a deletion date is then selected uniformly from the edges valid lifespan.

\paragraph{Delete Post/Comment and Delete Post/Comment Like}

\cite{DBLP:conf/cscw/AlmuhimediWLSA13} tracked 292K Twitter users for 1 week finding 2.4\% of 67.2M tweets were deleted across 4 categories: status posts, retweets, replies, and mentions of other users that were not replies.
In order to apply these findings to Datagen, tweet categories were mapped to Datagen message types.
\autoref{table:almuhimedi-mapping} the entity mapping and the percentage deleted across the simulation period within each category.
...
Thus, each \tPerson in the network has a 50\% chance of being marked a ``deleter'', who subsequently, may or may not, delete post, comments, or likes.
\cite{DBLP:conf/cscw/AlmuhimediWLSA13} also identify a relationship between the depth of comments on a post and the chance the post is removed, less comments, more likely a post is deleted.
We model this relationship using the distribution above.
\todo{add plot}

Posts in groups and walls are produced via a uniform generator and a flash mob generator, capturing background events and bursts in events respectively.
A comment generator is then used to produce a tree of comments on each post.
Posts in albums are referred to as photos.
They are produced by a different generator and do not have flash mob events nor do they have comment trees.
Additionally, each post and comment have a number of likes generated for it.
Almuhimedi et al. (2013) observes a temporal relationship for when a tweet is deleted.
We have recreated this distribution and use it to generate deletion dates for posts, comments, and likes that are selected for explicit deletion.
\todo{add plot}
...
\section{Serialization Constraints: Converting Delete Events into Delete Operations}
...
When run in Interactive mode Datagen produces a graph that monotonically increases in size over the simulation period with insert-only operations, e.g., once \tPerson joins the network they never leave, not delete a post nor unlike a picture.
...
For the BI mode the generated data must be converted into a bulk load component and a series of refresh batches (containing insert and d delete operations.
