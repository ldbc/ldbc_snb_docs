...
SFs are computed based on the ASCII size in Gibibytes of the generated output files using the CsvMergeForeign serializer and default settings (both the 90\% initial data and the 10\% update streams count towards the total size).
...
The following CSV variants are supported:
    \begin{itemize}
      ...
      (1)~additional attributes (e.g. \texttt{deletionDate}, \texttt{isExplicitlyDeleted}, \texttt{weight}) are included mainly for debugging,
      (2)~all data is included, i.e. if a Forum is created and deleted before sampling the initial data set, it is included in this data set.
