...
\begin{itemize}
	...
	\item Regular edges, \ie edges that must be present in the subgraph, are denoted with solid lines.
	...
	\item Aggregations are shown in boxes with a gray strip on their top describing the type of aggregation ($\mathsf{count}$, $\mathsf{sum}$, $\mathsf{avg}$, etc.).
\end{itemize}
