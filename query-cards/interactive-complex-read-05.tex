...
%
 ...
 \\ \hline
		$\mathsf{2}$ & postCount & 32-bit Integer & A &
				\texttt{postCount} -- Number of \emph{Posts} made in \emph{Forum} that
were created by friends
 \\ \hline
		\end{tabularx}}\innerCardVSpace \\ \hline
...
given \emph{Person}, moving to friends and friends of friends, and then
getting the \emph{Forums} they are members of. Besides testing the
ability of the query optimizer to select the proper join operator, it
rewards the usage of indexes, but their accesses will be presumably
scattered due to the two/three-hop search space of the query, leading to
unpredictable and scattered index accesses. Having efficient
