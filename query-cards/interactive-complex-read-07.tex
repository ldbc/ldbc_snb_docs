...
%
	pattern & \multicolumn{1}{c|}{\includegraphics[scale=\patternscale,margin=0cm .2cm]{patterns/interactive-complex-read-07}} \\ \hline
%
...
%
 ...
 \\ \hline
		$\mathsf{7}$ & minutesLatency & 32-bit Integer & C &
				\texttt{minutesLatency} -- Duration between creation of Message and
Like, in minutes
 \\ \hline
		$\mathsf{8}$ & isNew & Boolean & C &
				\texttt{isNew} -- \texttt{false} if liker Person is friend of start
Person, \texttt{true} otherwise
 \\ \hline
		\end{tabularx}}\innerCardVSpace \\ \hline
...
$ &  \\ \hline
	...
	%
	relevance &
		\footnotesize This query looks for paths of length two, starting from a given Person, moving
