...
\emph{This chapter is based on the TPCTC~2020 conference paper ``Towards Testing ACID Compliance in the LDBC Social Network Benchmark''~\cite{tpctc_acid}, co-authored by several members of the SNB task force.}
...
Currently, LDBC provides no mechanism for validating ACID compliance within the SNB-I workflow.
...
%which could cast doubt on any performance results obtained from SNB-I.
...
SNB-I does not require systems to provide \level{Serializability}~\cite{Angles2020}.
...
Local transactions (involving a single data item) occur frequently in graph databases, \eg in \emph{``Retrieve content of a message''} (SNB-I Short Read 4~\cite{Angles2020}).
...
Pattern matching is a common predicate read operation in a graph database, \eg query \emph{``Find friends and friends of friends that have been to given countries''} (SNB-I Complex Read 3~\cite{Angles2020}).
...
% For example, in the SNB-I schema~\cite{Angles2020} a \texttt{Forum} can only have a single \texttt{MODERATOR} edge.
...
While this paper mainly focused on \emph{atomicity} and \emph{isolation} from the ACID properties, we provide a short overview of the other two aspects.
\emph{Durability} tests are already part of the benchmark specification~\cite{Angles2020}, while adding complex \emph{consistency} checks is left for future work.

{\bf Durability} is a hard requirement for SNB-I and checking it is part of the 
auditing process. 
The durability test requires the execution of the SNB-I workload and uses the LDBC workload driver.
